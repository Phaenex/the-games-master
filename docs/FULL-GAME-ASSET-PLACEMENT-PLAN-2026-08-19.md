...
| Need | Status | Source and placement | Implementation notes |
|---|---|---|---|
| Pieces | `BUILD` | `Apt_Props_01.fbx` contains only "Chess Board", "Chess Board Base" and a single generic "Pawn" — no King/Queen/Rook/Bishop/Knight meshes exist anywhere in the local asset vault (verified 2026-08-20 via binary string search; a broader vault search for a complete chess set also found nothing). All three authored positions use only King/King/Queen/Rook/Pawn, so four piece-type silhouettes (not six) are procedurally authored as combined, non-primitive meshes, matching the modeled-pip precedent Bones set for its dice | Instantiate only the pieces the current position's FEN actually places; white/black distinguished by material, never color alone (icon + text evidence per the arbiter-override intervention) |
| Board | `BUILD` | Project-authored 8×8 surface (64 individual authored tiles, alternating light/dark). `Chess_Board_Base.prefab` and its `Chessboard.mat`/`Chessboard_01_A.png` texture exist in the source vault but have never been imported into `unity-project/Assets/ThirdParty/`; importing a new third-party pack is its own conversion/attribution/licensing pass and is deferred rather than opened mid-scaffold — candidate for a later visual-fidelity pass, not used here | Square IDs, legal moves, original position, and override position must remain exact and readable in high contrast |
