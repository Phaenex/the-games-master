...
| Board tiles built near the floor instead of on the table; separately, 64 chess pieces were floating ~0.87m above the board. Both invisible in every green test and every audit check — only the rendered tour screenshot caught either one | `GmOwnedPropFactory.CreateRoundedProp`/`CreateDisc` apply their `position` argument in **world** space via `Transform.SetPositionAndRotation`, not local space, despite reading like a local offset at the call site. `CreateLocalRoundedProp` exists specifically to wrap this safely but is easy to reach for the "obvious" unwrapped call instead. Separately, a piece's stacked disc sub-meshes (`StackDisc`) hit the identical mistake a second time in the same pass, in the same file, right after the first instance was fixed — and stayed invisible-safe only because the piece root happened to sit at world origin during construction, a coincidence the next scene to move that hierarchy would break | Never call `CreateRoundedProp`/`CreateDisc`/`CreateMeshObject` directly with an offset that is meant to be relative to a parent — always go through `CreateLocalRoundedProp`, or create at `Vector3.zero`/`Quaternion.identity` and assign `.localPosition`/`.localRotation` explicitly afterward (the pattern `CreateDieFacePips` already used). After fixing one instance of a bug class in a file, grep the same file for every other call to the same primitive that could carry the identical mistake — don't assume one fix closed the class. Add a dedicated world-height regression test for both the container-level object (board root, piece root) and the actual rendered geometry (tiles, pieces) — a composition-audit check on the container's own position does not prove its children are correctly placed. |
| Aldric's clause-swap intervention (Wager) could never actually open — `DeterministicGameSession.TryOpenIntervention` threw "must produce a different state fingerprint" every single time, even though the swap genuinely changes match state | The match's lightweight decision fingerprint deliberately excluded the data the swap changes, mirroring how `GmStudyMatch` excludes `currentFen` (authenticated instead via `TryRestore`'s full replay). But Study's fingerprint still changes across its own intervention because `correctCount` — a value the fingerprint *does* include — genuinely differs between the honest and altered moments. Wager's swap only touches contract values, which the fingerprint excluded entirely, so the "before" and "after" fingerprints were byte-identical by construction, not by accident. A second bug rode along: even after including the right value, the first fix recorded the decision with the *post*-swap fingerprint and then tried to open the intervention with the same value — the ordering has to be honest-fingerprint-recorded-first, swap, then altered-fingerprint-opens-intervention, matching `GmStudyMatch.OpenIntervention`'s exact call order | When a match's fingerprint deliberately omits some state for forgery-security reasons, include the *one specific field* that the intervention's before/after actually differ on — not the whole excluded blob, just enough to make `TryOpenIntervention`'s equality check meaningful. Before wiring a new game's intervention, check that some fingerprint-included field actually changes between the honest and altered branches; if the only thing that changes is in the excluded set, the guard will reject every attempt. And copy the exact call order from a working sibling (`GmStudyMatch.OpenIntervention`), not just the general shape — record-decision-with-honest-fingerprint has to happen *before* the state mutation whose result becomes the intervention's altered fingerprint. |
