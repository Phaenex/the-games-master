...
public sealed class GmWendRuntimeCulling : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            if (!renderer.enabled || renderer.GetComponent<Terrain>() != null ||
                renderer.transform.root.name == "GmWendOpening" ||
                renderer.transform.root.name == "WakeRoom" ||
                renderer.transform.root.name == "HouseBeginning" ||
        ...
        foreach (Light light in FindObjectsByType<Light>(FindObjectsInactive.Exclude, FindObjectsSortMode.None))
        {
            if (!light.enabled || light.type == LightType.Directional ||
                light.transform.root.name == "GmWendOpening" || light.transform.root.name == "WakeRoom" ||
                light.transform.root.name == "HouseBeginning") continue;
            lights.Add(light);
        }
        BeginSweep();
