...
public sealed class GmStandaloneReviewProbe : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        yield return new WaitForSecondsRealtime(0.45f);
        HideTransientReviewUi();
        yield return Capture("05-cemetery-composition.png");
    ...
    IEnumerator ValidateControllerInput(GmPlayer player, GmColdOpen cold)
    {
        ...
        try
        {
            ...
            var document = FindAnyObjectByType<GmPrologueHud>()?.GetComponent<UIDocument>();
            var prompt = document?.rootVisualElement.Q<Label>("Prompt");
            if (prompt == null || !prompt.text.Contains("A / CROSS") || !prompt.text.Contains("B / CIRCLE"))
            ...
            var pausedBody = document?.rootVisualElement.Q<Label>("CardBody");
            if (!brightnessAdjusted || pausedBody == null || !pausedBody.text.Contains("BRIGHTNESS"))
            ...
            if (controls == null || !controls.text.Contains("LEFT STICK / D-PAD") ||
                !controls.text.Contains("MENU / OPTIONS"))
