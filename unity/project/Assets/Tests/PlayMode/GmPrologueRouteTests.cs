...
public class GmPrologueRouteTests
{
    ...
    public IEnumerator VirtualGamepadDrivesTheCompletePhaseZeroControlSurface()
    {
        ...
        try
        {
            ...
            var document = FindBehaviour("GmPrologueHud")?.GetComponent<UIDocument>();
            Assert.IsNotNull(document);
            var prompt = document.rootVisualElement.Q<Label>("Prompt");
            Assert.IsNotNull(prompt);
            StringAssert.Contains("A / CROSS", prompt.text);
            StringAssert.Contains("B / CIRCLE", prompt.text);
            ...
            StringAssert.Contains("Y / TRIANGLE", prompt.text);
            ...
            StringAssert.Contains("LEFT STICK / D-PAD", controls.text);
            StringAssert.Contains("MENU / OPTIONS", controls.text);
        }
        finally
        {
