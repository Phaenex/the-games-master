...
// If the estate/grounds never finish building, every pose below shoots an unbuilt scene — that
// is a failed gate, not a warning, so it has to reach `pass`.
const readyOK = await page.waitForFunction(()=>window.__GMC._estateReady && window.__GMC._leartesGroundsReady, null, { timeout:120000 }).then(()=>true).catch(()=>false);
console.log(`estate/grounds ready: ${readyOK ? 'PASS' : 'FAIL'}`);
...
pass = errs.length === 0 && readyOK && walkedIn && examOK && rectsOK;
console.log(pass ? 'ESTATE CAPTURE PASS' : 'ESTATE CAPTURE FAIL');
} catch (e) {
  console.log('ESTATE CAPTURE FAIL — threw:', (e && e.message) || e);
} finally {
