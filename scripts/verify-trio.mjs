...
// A thrown assertion or a waitForFunction timeout must not leak the Chromium tree or leave PORT
// bound — the next run would then fail with EADDRINUSE instead of the real reason.
try {
// ─── Prologue car ───────────────────────────────────────────
{
...
// ─── Entry Hall Modular ─────────────────────────────────────
{
  ...
  // Wait for modular wall albedo maps (async TextureLoader) and for the wall meshes themselves —
  // modWalls is a pass/fail assertion below, so the count must not race the GLTFLoader.
  // {timeout} is the THIRD arg; as the second it lands in `arg` and the 30s default applies silently.
  await page.waitForFunction(() => {
    let mapped = 0, walls = 0;
    window.__GMC.scene.traverse((o) => {
      if (!o.isMesh) return;
      if (/Wall_01|wall_01|NoLedge/i.test(o.name || (o.parent && o.parent.name) || '')) walls++;
      if (!o.material) return;
      const ms = Array.isArray(o.material) ? o.material : [o.material];
      ms.forEach((m) => { if (m && m.map && m.map.image) mapped++; });
    });
    return mapped >= 2 && walls > 0;
  ...
  // The whole point of this section is the modular wall kit — a scene that boots with none of it
  // loaded is the regression to catch, and kids>20 is true of any hall that boots at all.
  results.push({ scene: 'hall', ok: hallInfo.modWalls > 0 && hallInfo.kids > 20 && errs.length === 0, hallInfo, errs });
  await page.close();
}
...
// Court and STB have no geometric assertion of their own, and "the agent will read the images" is a
// taste pass, not a measurement. Every frame this gate wrote goes through the repo's pixel scanner
// (magenta / near-black / blown / flat), which also fails when nothing was captured at all.
const scan = spawnSync('node', ['scripts/scan-frame-defects.mjs', ...shots], { stdio: 'inherit' });
const framesOK = scan.status === 0;
if (scan.error) console.log('frame scan could not run:', scan.error.message);

const failed = results.filter((r) => !r.ok);
console.log('\n=== TRIO GATE ===');
console.log(JSON.stringify(results, null, 2));
if (failed.length || !framesOK) {
  console.log('FAIL', [...failed.map((f) => f.scene), ...(framesOK ? [] : ['frame-defects'])].join(', '));
  process.exit(1);
}
