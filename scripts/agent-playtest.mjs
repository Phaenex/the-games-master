...
// an unhandled 'error' event on the server throws with a bare stack; say what it actually means
// (a leaked server from a crashed earlier run is the usual cause of EADDRINUSE here)
server.on('error', (e)=>{ console.error(`FATAL: static server on :${PORT} — ${e.code||e.message}${e.code==='EADDRINUSE'?' (a previous rig run leaked the port)':''}`); process.exit(1); });
...
async function walkthrough(page) {
  ...
  for (const w of route) {
    const r = await page.evaluate(async (w)=>{
      ...
      let moved = 0;
      for (const key of ['w','s','a','d']) {   // all four cardinals: a stop whose only opening is a left strafe is still walkable
  ...
  // gate-lock beat: turn back mid-drive. The route setup above force-set _gateLockFired so the
  // teleport route could stand on the estate side — leave it set and this check reads its own
  // setup and can never fail. Reset to the pre-lock state, then make the game fire it for real.
  await page.evaluate(()=>{
    const C = window.__GMC;
    C._gateLockFired = false; C.gateClosed = 0; C.gateTriggered = false;
    C._passedGate = true;                 // through the gate already: turning back must LOCK, not end the run
    C.cam.position.set(0,1.7,40); C.yaw = Math.PI;
    C._minZReached = 20;                  // simulate having walked deeper
    C.keys['w'] = true;
  });
  const lockFired = await page.waitForFunction(()=>window.__GMC._gateLockFired===true, null, { timeout:15000 }).then(()=>true).catch(()=>false);
  const gate = await page.evaluate(()=>{
    const C = window.__GMC; C.keys['w'] = false;
    const g = { lockFired: !!C._gateLockFired, gateClosed: +(C.gateClosed||0) };
    C.setState({ beatOn:false });          // don't leave the slam card over the arrival screenshots
    return g;
  });
  log(`  gate-lock beat: ${gate.lockFired ? 'fired' : 'NOT FIRED'} · gateClosed=${gate.gateClosed}`);
  if (!lockFired || !gate.lockFired)
    F('HIGH','walkthrough','gate-lock-never-fired','gate', 'walked back past the deepest point with _passedGate set and the lock never armed', 'check the movingBackward branch in the walk loop / triggerGateLock');
  else if (gate.gateClosed < 0.99)
    F('HIGH','walkthrough','gate-not-closed','gate', `lock armed but gateClosed=${gate.gateClosed} (leaves not shut)`, 'triggerGateLock must close the gate — Threshold Refusal depends on it');
...
async function scenes() {
  ...
  for (const s of list) {
    ...
    let shot = 0;
    // a throw inside goTo() comes back as a REJECTED evaluate on this side, never as a pageerror —
    // swallowing it counted a phase that was never entered. Collect the failures and report them.
    const phaseFails = [];
    for (const ph of (phases||[]).slice(0,6)) {
      const goErr = await page.evaluate((n)=>window.__GM.goTo(n), ph).then(()=>null, e=>e.message);
      if (goErr) { phaseFails.push(`goTo('${ph}'): ${goErr}`); continue; }
      await page.waitForTimeout(ph==='wake'?4200:800);
      const shotErr = await page.screenshot({ path:`${DIR}/apt-${s.tag}-${ph}.png`, timeout:60000 }).then(()=>null, e=>e.message);
      if (shotErr) { phaseFails.push(`screenshot('${ph}'): ${shotErr}`); continue; }
      shot++;
    }
    log(`  ${s.tag}: booted=${booted} phases-shot=${shot}/${(phases||[]).slice(0,6).length} errors=${errs.length} phase-failures=${phaseFails.length}`);
    ...
    if (phaseFails.length) F('HIGH','scenes','phase-failed', s.tag, phaseFails.slice(0,2).join(' | '), 'the dev phase bridge threw — run __GM.goTo by hand in that scene');
    out.push({ scene: s.tag, booted, errors: errs.length, phasesShot: shot, phasesFailed: phaseFails.length });
    await page.close();
  }
  return out;
}
...
// everything below runs inside try/finally: a throw anywhere used to leave Chromium and the
// :3830 server alive (so the NEXT run died on EADDRINUSE) and skipped the report write, losing
// every finding collected up to that point.
let fatal = null;
try {
  ...
  fatal = e;
  F('HIGH','rig','run-crashed','agent-playtest', String((e && e.stack) || e).split('\n').slice(0,3).join(' | '), 'the rig itself threw — the sections below it never ran');
} finally {
  await browser.close().catch(()=>{});
  await new Promise(r=>server.close(()=>r()));
}
report.fatal = fatal ? String((fatal && fatal.stack) || fatal) : null;
...
if (fatal) log(`  RIG CRASHED: ${fatal.message || fatal}`);
process.exit(report.counts.HIGH > 0 || fatal ? 1 : 0);
