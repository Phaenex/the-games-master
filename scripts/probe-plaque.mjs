...
// Own filename on purpose: estate-12-gate-plaque.png is capture-estate.mjs's canonical
// grounds-tour frame at its own pose, and this probe stands somewhere else.
const SHOT = 'probe-gate-plaque';
...
try {
  ...
  await page.screenshot({ path:`${DIR}/${SHOT}.png`, timeout:60000 });
  console.log(`shot ${SHOT} | errors:`, errs.length);
} finally {
  await browser.close(); server.close();
}
if (!info || !info.found) { console.log('probe-plaque FAILED | no gatePlaque in scene'); process.exitCode = 1; }
if (errs.length) { console.log('probe-plaque FAILED |', errs.length, 'pageerror(s)'); process.exitCode = 1; }
