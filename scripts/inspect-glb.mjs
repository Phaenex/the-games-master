...
for (const f of files.sort()) {
  let row;
  try { row = await inspect(f); }
  catch (e) { row = { path: f, error: String(e.message || e) }; }
  if (row.error) failed++;
  console.log(JSON.stringify(row));
}
// The rows are readable on their own, but the exit code is what a caller can gate on.
if (failed) { console.error(`inspect-glb: ${failed}/${files.length} file(s) failed to parse`); process.exitCode = 1; }
