...
const PREAMBLE = `
PROJECT: The Games Master — Unity 6000.5.3f1 / HDRP Steam horror game at /Users/damato/GamesMaster-Unity.
...
2. SANDBOX: /Users/damato/GamesMaster-Unity is OUTSIDE the Bash sandbox write allowlist. EVERY Unity
   command and EVERY Blender command needs dangerouslyDisableSandbox: true. Writes to the web repo are fine sandboxed.
3. UNITY IS SINGLE-INSTANCE. Only drive it via 'node scripts/unity-cli.mjs <task>' from the web repo.
...
COMMANDS (from the web repo, sandbox OFF):
  node scripts/unity-cli.mjs rebuild | tour | test
Screenshots land at ~/GamesMaster-Unity/Screens/WendHill/tour-*.png (12 of them). Logs at ~/GamesMaster-Unity/Logs/.
...
clock: find ~/Projects/the-games-master/assets/models/unity -iname '*clock*'. If none, build a primitive
...
tour confirming the 12 grounds shots are UNCHANGED (room isolated).`,
...
2. Confirm the room is at z~+400 and the 12 grounds shots are UNCHANGED (no interior walls leaking in).
...
(unity/design-data/prologue-design.json AND ~/GamesMaster-Unity/Assets/StreamingAssets/prologue-design.json —
...
3. node scripts/unity-cli.mjs tour, then OPEN ALL TWELVE PNGs and LOOK at each.
...
Report every command's real numbers, a per-shot verdict for all 12, and the ranked list. Confirm nothing committed.`,
