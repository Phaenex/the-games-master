...
// A NaN batch size makes need.slice(i, i + NaN) an empty chunk, so the convert loop would run once,
// dispatch nothing and report success. Reject the bad flag instead of silently converting zero files.
const batchArg = args.find((a, i) => args[i - 1] === '--batch');
const BATCH = batchArg === undefined ? 40 : Number(batchArg);
if (!Number.isInteger(BATCH) || BATCH < 1) {
  console.error(`--batch expects a positive whole number, got '${batchArg}'`);
  process.exit(2);
}
...
console.log('\nSTATS', stats);
// Keeping going past a bad package was the point, so the loops above never abort — but a run that
// failed every conversion (missing fbx2gltf, unreadable package) must not report success to its
// caller. The batch is the thing that continues; the exit code still tells the truth about it.
const failures = stats.extractFail + stats.convertFail;
if (failures) {
  console.error(`\nFAILED ${failures} item(s): extract=${stats.extractFail} convert=${stats.convertFail}`);
}
