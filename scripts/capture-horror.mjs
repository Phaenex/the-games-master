...
// Nine posed shots with the RAF cancelled: without this listener a throw from a GLTF callback
// or an event handler leaves nine plausible-looking frames and no signal at all.
...
// Shots 04/05/09 are porch fixtures — an unbuilt fixture makes those frames worthless.
...
if (!porchOK) errs.push('porch sconces never built (env-lamp|bracket)');
...
const shot = async (name, fn) => {
  ...
  try {
    ...
    // No canvas means the scene never came up; record it rather than letting the run end clean.
...
pass = errs.length === 0;
console.log(pass ? 'HORROR GATE PASS' : 'HORROR GATE FAIL');
} catch (e) {
  console.log('HORROR GATE FAIL — threw:', (e && e.message) || e);
} finally {
