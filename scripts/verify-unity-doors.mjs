...
await page.waitForFunction(() => window.__GMC && window.__GMC.scene && window.__GMC.mansionModel && window.__GMC.doorL && window.__GMC._unityFrontDoors, null, { timeout: 120000 });
...
// Door_Double hinged leaves on Prologue facade (night MeshBasic).
const ok = info.unity && info.leftSize && info.leftSize[1] > 1.8 && info.leftSize[1] >= info.leftSize[0] * 0.9;
console.log(ok ? 'VERIFY PASS (Door_Double facade leaves)' : 'VERIFY FAIL', info);
process.exit(ok ? 0 : 1);
