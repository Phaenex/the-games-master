...
const pageErrors = [];
const assetFailures = [];
// Everything is logged; only the estate's own broken assets decide the verdict. Same-origin because
// the webfont/CDN hosts are somebody else's uptime; not favicon.ico because Chromium asks this
// server for one the page never linked; not ERR_ABORTED because the ambience <audio> elements
// cancel their own range requests, which is the media stack working, not a missing file.
const ORIGIN = `http://localhost:${PORT}/`;
const gameAssetFailures = [];
const noteFailure = (url, line, aborted=false)=>{
  assetFailures.push(line);
  if (!aborted && url.startsWith(ORIGIN) && !url.endsWith('/favicon.ico')) gameAssetFailures.push(line);
};
page.on('pageerror', e=>{ pageErrors.push(e.message); console.log('[pageerror]', e.message); });
page.on('requestfailed', req=>{ const why = req.failure()?.errorText || 'request failed'; noteFailure(req.url(), `${req.url()} :: ${why}`, /ERR_ABORTED/.test(why)); });
page.on('response', res=>{ if(res.status() >= 400) noteFailure(res.url(), `${res.status()} ${res.url()}`); });
...
// Read presence separately from rotation. `doorL?.rotation.y || 0` reports a perfectly shut door
// for a door that isn't there, so a lost hinge reference would pass the refusal check instead of
// failing it — the one regression this gate exists to catch.
const doorState = await page.evaluate(()=>{ const C=window.__GMC; return {
  present: !!(C.doorL && C.doorR),
  left: C.doorL ? +C.doorL.rotation.y.toFixed(4) : null,
  right: C.doorR ? +C.doorR.rotation.y.toFixed(4) : null
}; });
console.log('FINAL', JSON.stringify(fin), 'DOORS', JSON.stringify(doorState), 'ASSET FAILURES', gameAssetFailures.length);
if (pageErrors.length) throw new Error(`page errors: ${pageErrors.join(' | ')}`);
// The listeners run for the whole walk, so this covers the cemetery/garden/porch stretch too — a
// texture or audio file that 404s at the far end of the drive fails the run, it does not just log.
if (gameAssetFailures.length) throw new Error(`game asset requests failed: ${gameAssetFailures.join(' | ')}`);
if (fin.phase !== 'aftermath' || fin.errors !== 0) throw new Error(`unexpected final state: ${JSON.stringify(fin)}`);
if (!doorState.present) throw new Error(`front door hinges missing — Threshold Refusal unverifiable: ${JSON.stringify(doorState)}`);
