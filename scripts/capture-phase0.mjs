...
// A readiness wait that times out means the shot list below is a picture of a half-built estate.
// Record it and keep capturing (the PNGs still show what went wrong) but never report that as a pass.
const notReady = [];
const ready = async (label, fn, timeout) => {
  await page.waitForFunction(fn, null, { timeout }).catch(() => {
    notReady.push(`${label} never became ready within ${timeout}ms`);
...
await ready('car + mansion GLTFs', () => window.__GMC && window.__GMC.carModel && window.__GMC.mansionModel, 20000);
await ready('ruins_pack grounds', () => window.__GMC && window.__GMC._ruinsBuilt !== undefined, 25000);
...
if (notReady.length) throw new Error(`scene never finished loading: ${notReady.join(' | ')}`);
