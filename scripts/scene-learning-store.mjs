...
// Polarity leads the signature. Without it a 'keep' and a 'reject' that happen to share a tag string
// aggregate into one candidate, and a Nick-promoted rule would permanently carry evidence pointing
// both ways with nothing on it recording which direction it means. 'change' and 'reject' stay one
// bucket on purpose: both mean "this wants fixing", which is the same split the affinity and defect
// paths already use.
function signatureOf(verdict) {
  const tag = [...(verdict.tags ?? [])].sort()[0] ?? verdict.verdict;
  const polarity = verdict.verdict === 'keep' ? 'keep' : 'fix';
  return `${polarity}|${verdict.category}|${verdict.context?.assetRole ?? '*'}|${verdict.context?.assetFamily ?? '*'}|${tag}`;
}
...
export function writeReviewSessionAtomic(root, session) {
  ...
  try {
    ...
    try {
      // linkSync refuses an existing destination; renameSync silently overwrites one. Between the
      // existsSync check above and this publish another process can land the same sessionId, and a
      // ledger whose whole point is immutability must raise that, not quietly lose a session.
