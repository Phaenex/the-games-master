...
// the floor applies to BOTH lamps — the dying one is the only one that ever swings near zero,
// so min2 >= 0 is the assertion that actually guards the negative-intensity regression.
const lampOK = result.mean2 < result.mean1 - 0.2 && result.min2 <= 0.1 && result.min2 >= 0 && result.min1 >= 0;
...
pass = bellOK && lampOK && layersOK && errs.length === 0 && result.errors === 0;
