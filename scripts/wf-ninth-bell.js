...
const PREAMBLE = `
PROJECT: The Games Master — Unity 6000.5.3f1 / HDRP Steam game.
Repo (design source, scripts, docs, authored C#) at /Users/damato/Projects/games/the-games-master. The
Unity project is INSIDE that repo at unity-project/ — there is no separate ~/GamesMaster-Unity tree.
Authored source lives in unity/ and syncs into unity-project/. Edit the repo copy, never the synced copy.
...
2. SANDBOX: unity-cli.mjs shells out to 'ps' for its editor-closed guard, and a sandbox that blocks
   ...
   Blender/python-writing-there command needs dangerouslyDisableSandbox: true.
3. UNITY IS SINGLE-INSTANCE. Only ever drive it via 'node scripts/unity-cli.mjs <task>' from
   /Users/damato/Projects/games/the-games-master. Never launch two at once.
...
COMMANDS (from /Users/damato/Projects/games/the-games-master, sandbox OFF):
  node scripts/unity-cli.mjs rebuild | tour | test
  npm run test:all
Logs: unity-project/Logs/cli-<task>.log · tour shots: unity-project/Screens/WendHill/tour-*.png
The tour waypoint table (unity/project/Assets/Scripts/GmShotTour.cs) is 18 shots, 01-spawn to
18-figure-cutoff-off.
...
Create scripts/gen-bell-audio.py EXACTLY as the plan specifies (complete python is in the plan) and run:
  python3 scripts/gen-bell-audio.py unity-project/Assets/Resources/Sfx
...
Search the owned library for a clock:
  find /Users/damato/Projects/games/the-games-master/assets/models/unity -iname '*clock*'
...
4. Run the tour and confirm the 18 grounds shots are UNCHANGED by this — no interior walls leaking in.
...
unity-project/Assets/StreamingAssets/prologue-design.json) — the plan has the exact JSON — then
...
Assets/Tests/EditMode/Editor/GmEstateBuildTests.cs — BellSummonsAndCrossingExist, BellCadenceIsShippable,
...
6. node scripts/unity-cli.mjs tour, then OPEN ALL EIGHTEEN PNGs and LOOK at each. Not a sample.
...
Report every command's real numbers, a per-shot verdict for all 18, and a RANKED list of what is
