...
// A thrown assertion or a wait timeout must not leak the Chromium tree or leave PORT bound.
try {
...
// Swallowing these waits meant a beat that never happened looked identical to one that did. The
// outcome is recorded and gated below. Budgets match verify-breath.mjs — the software renderer
// glides to the porch far slower than real time, and 20s there was the harness, not the game.
const reachedArrival = await page.waitForFunction(() => window.__GMC.mode === 'arrival', null, { timeout: 60000 })
  .then(() => true).catch(() => false);
await page.keyboard.up('w');
const beatFired = await page.waitForFunction(() => window.__GMC._glimpseSaid === true || window.__GMC.knockStarted === true, null, { timeout: 60000 })
  .then(() => true).catch(() => false);
...
results.prologue.trap.reachedArrival = reachedArrival;
results.prologue.trap.beatFired = beatFired;
// Closed-door trap: leaves must stay ~0 during hold/knock (no open fashion show). The rotations are
// NOT forced shut here any more — posing the leaves closed before the shot guaranteed the frame
// showed a pass whatever the game had just done. Both the frame and trap.doorRot now report reality.
await page.evaluate(() => {
  const C = window.__GMC;
  C.walkEnabled = false;
  if (C._raf) { cancelAnimationFrame(C._raf); C._raf = null; }
...
// freezePose cancelled the RAF, and goTo('rigged') only sets cam.position + yaw/pitch — the loop that
// copies those onto cam.rotation and re-renders is gone. Without this the "evidence" frame is a copy
// of shot 05, taken before presentTrue() tarnished the gavel.
await page.evaluate(() => {
  const C = window.__GMC;
  C.cam.rotation.set(C.pitch || 0, C.yaw || 0, 0, 'YXZ');
  C.renderer.render(C.scene, C.cam);
});
...
// Same stale-frame trap: the RAF is cancelled and demoShut9() syncs the board mesh without rendering,
// so the shut-9 frame would otherwise show the board before the tile folded.
await page.evaluate(() => {
  const C = window.__GMC;
  C.renderer.render(C.scene, C.cam);
});
...
// results.prologue.doorRot never existed — the trap capture writes results.prologue.trap.doorRot, so
// this line threw a TypeError before the verdict ever printed. It also asserted the leaves had SWUNG
// (>0.4), the opposite of the Threshold Refusal it sits under: the porch beat ends with the doors
// shut. 0.05 rad is a rounding tolerance on a value the harness reads to two decimals, not a feel
// value — nothing in the Prologue rotates these leaves at all.
const shut = (r) => Math.abs(r) < 0.05;
const pass =
  results.prologue.unityDoors && results.prologue.unityDoors.flag && results.prologue.unityDoors.h > 1.8 &&
  results.prologue.trap && results.prologue.trap.reachedArrival && results.prologue.trap.beatFired &&
  shut(results.prologue.trap.doorRot.L) && shut(results.prologue.trap.doorRot.R) &&
