...
def main() -> int:
    ...
    if chapel.exists():
        ...
        # The pack lives outside this repository, so absence is expected on a fresh machine and is
        # not an error here — but it is one cue short of the set, and that has to be said out loud.
        print(
            f"[missing] {chapel}  licensed Horror Elements source not present; "
            "the cue set below is built without it",
            file=sys.stderr,
        )
    ...
    built = ["clock_chime", "heartbeat", "whisper_bed", "ear_whine"]
    if chapel.exists():
        built.insert(0, "chapel_bell")
    for name in built:
        path = out / f"{name}.ogg"
